...
suppressPackageStartupMessages({
  ...
  library(exiftoolr)
  library(tictoc)
  library(fs) # Better file operations
  library(glue) # Better string interpolation
...
CONFIG <- list(
  ...
  ai_classification_csv = "coding/july_2025_pipeline/config_2025-07-28.csv",
  parent_dir = "E:/wd_camera_test/",
  remote_dir = "/home/willwright/Documents/wild_deserts_outputs/",
  image_width = 2048,
  image_height = 1440,
  ...
  output_csv_name = paste0("processed_camera_trap_data", today(), ".csv")
)
...
get_activity_statistics <- function(df) {
  ...
  cleaned_by_camera |>
    ...
    write.csv(CONFIG$output_csv_name)
}
...
if (interactive()) {
  ...
  write.csv(
    file_paths,
    paste0("validated_config", today(), ".csv"),
    row.names = FALSE
  )
  get_activity_statistics(final_results[[2]])
} else {
  cat("Run process_camera_trap_data() to execute the pipeline\n")
}
