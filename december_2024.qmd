...
time_thresholds <- c(
  ...
  "Quoll" = 300,
  "Bilby" = 300
) # Can be different per species


confidence_threshold <- 0.85 # Can be difference per species and per model (night_quolls_bilbies_blobs and day_v3)
...
all_species <- c("Kangaroo", "Cat", "Unidentifiable", "Rabbit", "Fox", "Quoll", "Bilby") # Add any others
...
evaluate_parameters <- function(params) {
  ...
  cleaned <- combined_df %>%
    ...
    filter(
      (model_type == "day" & confidence > params[[paste0("conf_day_", species)]]) |
        (model_type == "night" & confidence > params[[paste0("conf_night_", species)]])
    )
  ...
  for (cam in cameras) {
    ...
    if (nrow(temp) == 0) next # Skip if no data for this camera after filtering
    ...
    if (nrow(temp) > 1) {
      for (i in 2:nrow(temp)) {
        ...
        last_same_species_time <- tail(temp$datetime[1:(i - 1)][temp$species[1:(i - 1)] == current_species], 1)
        ...
        if (length(last_same_species_time) == 0 || difftime(temp$datetime[i], last_same_species_time, units = "secs") > threshold) {
  ...
  if (nrow(cleaned_by_camera_df) == 0) {
    ...
    finalised_ai <- data.frame(Species = character(), Date = as.Date(character()), count = integer(), method = character(), cumulative_count = integer())
  } else {
    finalised_ai <- cleaned_by_camera_df %>%
      ...
      ungroup() %>%
      tidyr::complete(Species = all_species, Date = seq(dmy("01-04-2024"), dmy("15-04-2024"), by = "day"), fill = list(count = 0)) %>%
  ...
  comparison_df <- full_join(ai_counts, manual_counts, by = c("Species", "Date")) %>%
    mutate(
  ...
  total_absolute_error <- sum(abs(comparison_df$ai_count - comparison_df$manual_count))
...
param_ranges <- list(
  ...
  conf_day_min = 0.8, conf_day_max = 0.95,
  conf_night_min = 0.8, conf_night_max = 0.95, # Night might need lower threshold?
  # Time buffer: e.g., 15 seconds to 10 minutes (600 seconds)
  time_min = 15, time_max = 600
)
...
for (i in 1:n_iterations) {
  ...
  current_params <- list()
  for (sp in all_species) {
    current_params[[paste0("conf_day_", sp)]] <- runif(1, param_ranges$conf_day_min, param_ranges$conf_day_max)
    current_params[[paste0("conf_night_", sp)]] <- runif(1, param_ranges$conf_night_min, param_ranges$conf_night_max)
    # Time threshold - often better to sample on log scale or use integers
    current_params[[paste0("time_", sp)]] <- round(runif(1, param_ranges$time_min, param_ranges$time_max))
  }
...
optimised_time_thresholds <- best_params_df[grepl("time_", best_params_df$parameter), ]
optimised_conf_thresholds <- best_params_df[grepl("conf_", best_params_df$parameter), ]
